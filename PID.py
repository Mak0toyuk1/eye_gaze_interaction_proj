...
OUT_BLOCK = OUTPUT_DIR / "pid_head_eye_block_results_fixed.csv"
OUT_PARTICIPANT_CONDITION = OUTPUT_DIR / "pid_head_eye_participant_condition_results_fixed.csv"
OUT_GLOBAL = OUTPUT_DIR / "pid_head_eye_friedman_global_tests_fixed.csv"
OUT_PAIRWISE = OUTPUT_DIR / "pid_head_eye_pairwise_wilcoxon_tests_fixed.csv"
OUT_SUMMARY = OUTPUT_DIR / "pid_head_eye_condition_summary_fixed.csv"
...
def pid_two_sources_imin(source_1: np.ndarray, source_2: np.ndarray, target: np.ndarray) -> dict:
    """
    Exploratory two-source PID using a simple I_min-style decomposition.

    This is not BROJA PID. It is a discrete histogram-style approximation:

        total MI = I(S1, S2 ; T)
        redundancy = min(I(S1 ; T), I(S2 ; T))
        unique S1 = I(S1 ; T) - redundancy
        unique S2 = I(S2 ; T) - redundancy
        synergy = total MI - unique S1 - unique S2 - redundancy
    """
    ...
    redundancy = min(source_1_mi, source_2_mi)
    unique_1 = source_1_mi - redundancy
    unique_2 = source_2_mi - redundancy
    synergy = total_mi - unique_1 - unique_2 - redundancy
...
def add_lagged_states(frames: pd.DataFrame) -> pd.DataFrame:
    frames = frames.sort_values(
        ["participant_id", "layout", "tracking", "trial_index", "timestamp"]
    ).copy()
