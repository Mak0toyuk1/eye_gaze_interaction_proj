...
# None means use every tracking mode found in balanced_subject_block_summary.csv.
# This is needed to test bias-present vs bias-absent and jitter/noise-present vs absent.
TRACKING_MODES = None
...
def main() -> None:
    log("Starting all-modes histogram-based pairwise yaw/pitch TE script")
    ...
    if TRACKING_MODES is None:
        detected_modes = sorted(block_summary["tracking"].dropna().unique().tolist())
        log(f"Detected tracking modes: {detected_modes}")
        blocks = block_summary.copy()
    else:
        detected_modes = TRACKING_MODES
    ...
    mode_label = "all_modes" if TRACKING_MODES is None else "_".join(TRACKING_MODES)

    suffix = (
        f"diagnostic_{mode_label}_pairwise_2d_{N_BINS_PER_DIM}x{N_BINS_PER_DIM}_"
